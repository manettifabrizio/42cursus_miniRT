...
//
// c++ -o camerarays camerarays.cpp -O3 -std=c++11 -D MAYA_STYLE
//
...
void render(
    ...
    const std::vector<std::unique_ptr<Light>> &lights)
{
    Matrix44f cameraToWorld; //definito come struttura
    Vec3f *framebuffer = new Vec3f[options.width * options.height]; //alloca spazio
    ...
    // [/comment]
    Vec3f orig;
    cameraToWorld.multVecMatrix(Vec3f(0), orig); // moltiplicazione punto-matrice
    for (uint32_t j = 0; j < options.height; ++j) {
        for (uint32_t i = 0; i < options.width; ++i) {
            ...
            // [/comment]
            Vec3f dir;
            cameraToWorld.multDirMatrix(Vec3f(x, y, -1), dir); //multiplicazione vettore-matrice
            dir.normalize();
